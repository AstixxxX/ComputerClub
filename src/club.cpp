...
void Club::ClientSitsDownAtTable(const std::string& client, const std::string& strTime, int tableId) 
{
    ...
    if (clients_.contains(client)) 
    {
        if (tables_.at(tableId).IsVacant()) 
        {
...
void Club::ClientWaits(const std::string& client, const std::string& strTime) 
{ 
...
void Club::ClientLeaves(const std::string& client, const std::string& strTime) 
{
    ...
    if (clients_.contains(client)) 
    {
        ...
        {
            events_.push_back(strTime + " 12 " + waitingQueue_.front() + " " + std::to_string(tableId + 1));
...
void Club::PrintReport() 
{
    ...
    for (int i = 0; i < tables_.size(); ++i) 
        std::cout << i + 1 << " " << tables_[i].GetIncome() << " " << tables_[i].GetWorkTime() << std::endl;
}
